...
STL在接口上并没有提供`std::advance_random`这样的接口，而是使用了统一的接口，这意味着这个算法的内部必须判断输入的迭代器属于什么类别的迭代器（注意用词，它需要知道迭代器属于什么类别而不是迭代器到底是什么类型，比如`int*`的类型是int指针，而迭代器分类中属于随机迭代器，下文中的表述除非特殊说明，否则说的都是类别classification而不是类型type）。

传统的OO思维类型判断可能是提供一个方法返回类型，但是STL并不是这样做的，它通过另外一种更加灵活的技术——`Traits`——来实现迭代器所属类别的判断。

## std::iterator_traits

`traits`技术的一个很实用的模板技术，它通常用于特征或者说的提取。`std::iterator_traits`是标准库中提供的`traits`之一用于提取迭代器的特征（其他的还有`char_traits`）。这个类通常内部至于类型定义，没有实际的数据成员和方法（`char_traits`有成员方法，有成员方法的通常被称为`Policy`）。
...
std::hash中使用到的特化技术是全特化技术，而这里使用到的偏特化技术（他们之间的区别请参考《C++模板》一书）。

iterator_traits定义了下面五个类型成员（member type）
...
其中最后一个类型可以用于标识迭代器的实际分类，你比如对于指针的特化中，上面这几个类型被定义为下面的类型：
...
指针的迭代器分类被定义为：`std::random_access_iterator_tag`，这是一个很有意思的话题，因为它用类型来表示来Concept，而前面提到Concept并不是一个类型而是符合某些条件的类型的集合。因为它不是具体的类型，所以这些tag实际上只做类型区分作用，并没有任何的成员，而Concept，Refinment的关系实际上也被表示为空类的父子继承关系：
...
在traits的特化的时候，我们可以用其中一个类型来表示实际的迭代器的分类。
...
前面提到，充分利用迭代器的分类信息可以提升算法的性能，而算法利用分类信息的方式通常是函数重载，我们回头来看`std::advance`的实现方式：
...
迭代器是指针的泛化版本，它实际上遗传来指针的一些问题，比如野指针，这个问题在插入和删除的时候非常常见，比如在vector尾部插入数据，可能导致底层数组的重新分配，它会导致之前的迭代器失效，类似野指针。
...
Mode`，不同的编译器提供来不同的方式开启Safe Mode，比如GCC中可以使用`-D_GLIBCXX_DEBUG`来开启，MSVC在Debug模式下会默认执行检测。
...
`std::back_inserter`实际上是一个工厂方法（参考前面`make_pair`），它帮我们创建一个`back_inserter_iterator`。而`back_inserter_iterator`是一个迭代器适配器。
...
对于字符串的处理，`istringstream`配合算法可以达到非常好的效果。

### 逆序迭代器

`reverse_iterator`可以把正向的遍历变成逆序遍历，比如查找，默认查找第一个，但是如果我使用`reverse_iterator`适配一下，我可以轻易的实现查找最后一个元素的功能。

逆序迭代器在实现上有一个难点就是区间，实际上都是定义为`[begin, end)`，这种形式，读取begin合法而读取end不合法。但是区间如果反过来，`(begin -1, end -1 ]`这个区间却不一定合法，因为`begin - 1`不是一个合法的迭代器。为了解决这个问题，逆序迭代器中的元素物理位置和逻辑位置有一个元素的偏差。物理上我们依旧使用`[begin, end)`这个区间，逻辑上对`end` 的操作存取的是`end - 1` 这个位置。物理位置到逻辑位置的转换可以通过 `base` 成员返回。（更详细的细节可以参考[标准](http://devdocs.io/cpp/iterator/reverse_iterator)，或者《Effctive STL》一书）。
